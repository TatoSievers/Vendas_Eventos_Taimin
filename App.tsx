...
const App: React.FC = () => {
  ...
  // Load sales data from Supabase on initial mount, only if authenticated
  useEffect(() => {
  ...
  const handleSaveSale = async (saleData: SalesData, isEditing: boolean) => {
    // FIX: Removed `observacao` property as it does not exist on the SalesData type and is not captured in the form.
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-gradient-to-br from-slate-900 to-slate-700 text-gray-200">
        <div className="text-center bg-slate-800 p-8 rounded-lg shadow-2xl">
          ...
          <p className="text-lg text-gray-300 mt-2">A senha de acesso da aplicação não foi configurada no ambiente.</p>
  ...
  if (!isAuthenticated) {
    return <PasswordScreen appPassword={appPassword} onCorrectPassword={() => setIsAuthenticated(true)} />;
  }
