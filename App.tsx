...
const App: React.FC = () => {
  ...
  const handleSaveSale = async (saleData: SalesData, isEditing: boolean) => {
    // Create a clean payload for the 'sales' table.
    // This removes fields that are handled separately or are auto-generated by the database.
    const salePayload = { ...saleData };
    delete salePayload.produtos;     // Handled in sale_products table.
    delete salePayload.created_at;   // Should not be sent; DB manages it.
    delete salePayload.id;           // The record's PK, not part of the SET clause.
